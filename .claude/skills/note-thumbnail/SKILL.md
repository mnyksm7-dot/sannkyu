---
name: note-thumbnail
description: "note.com 記事用のサムネイル(見出し画像)を作成する。ユーザーが無料ツール(Gemini アプリ、Bing Image Creator など)で作った画像を渡してくれたら、scripts/generate-thumbnail.ts で note.com 推奨サイズ(1280x670)にリサイズ・クロップする。GEMINI_API_KEY が課金済みであれば、画像生成そのものも自動化できる。「サムネを作って」「サムネ作成自動化」「/note-thumbnail」のように呼ばれたときに使う。note.com への画像アップロード自体は行わない。"
---
